...
#if WITH_EDITOR
static UGameplayEffect* CreateOrLoadGameplayEffectAsset(const FString& AssetPath, const FString& AssetName)
{
    ...
    // If already exists in package, return it
    if (UGameplayEffect* Existing = FindObject<UGameplayEffect>(Package, *AssetName))
    {
        Existing->Modify();
        return Existing;
    }
    ...
    // Create new asset
    UGameplayEffect* GE = NewObject<UGameplayEffect>(Package, *AssetName, RF_Public | RF_Standalone);
    if (!GE)
        return nullptr;
...
)
{
    ...
    UGameplayEffect* GE = CreateOrLoadGameplayEffectAsset(AssetPath, AssetName);
    if (!GE)
        return false;
